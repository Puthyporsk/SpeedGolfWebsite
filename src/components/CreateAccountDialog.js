...
class CreateAccountDialog extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {checkValid: false};
        if (!this.props.accountClicked)
        {
    ...
    checkAccountValidity = () => {
        if (this.props.userId !== this.newUserRef.current.value)
            this.checkValidOn();
    ...
    handleEditAccount = (event) => {
        ...
        let old = this.props.userId;

        this.props.setUserId(this.state.accountName);

        data.displayName = this.state.displayName;
        this.props.changeDName(data.displayName);

        data.password = this.state.accountPassword;

        data.profilePicDataURL = this.state.profilePicDataURL;
        data.profilePicURL = this.state.profilePicURL;
        if (data.profilePicDataURL !== "")
            this.props.changePfp(data.profilePicDataURL);
        else 
            this.props.changePfp(data.profilePicURL);
        
        data.securityAnswer = this.state.accountSecurityAnswer;
        data.securityQuestion = this.state.accountSecurityQuestion;

        //Commit to local storage
        localStorage.removeItem(old);
        localStorage.setItem(this.state.accountName,JSON.stringify(data));

        this.props.toggleAccountClicked();
    ...
    <div className="modal" role="dialog">
        ...
        <div className="modal-content">
            ...
            <center>
                <h3 className="modal-title">{this.props.accountClicked ? <b>Edit Account</b>: <b>Create New Account</b>}</h3>
            </center>
            <button className="close" 
                onClick={this.props.accountClicked ? this.props.toggleAccountClicked : this.props.cancelCreateAccount}>
                &times;</button>
            </div>
            <div className="modal-body">
            <form onSubmit={this.props.accountClicked ? this.handleEditAccount : this.handleCreateAccount}>
            <label>
                Email: 
                <input
            ...
            <button role="submit" 
                ...
                <span className="fa fa-user-plus"></span>{this.props.accountClicked ? <b>&nbsp;Update Account</b>: <b>&nbsp;Create Account</b>}
            </button>
            </form>
            </div>
        </div>
