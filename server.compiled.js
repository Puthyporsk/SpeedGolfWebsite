...
import express from 'express'; // require('dotenv').config();

const LOCAL_PORT = 8080; // const DEPLOY_URL = "http://localhost:8080";
...
async (accessToken, refreshToken, profile, done) => {
  ...
  const userId = `${profile.username}@${profile.provider}`; //See if document with this unique userId exists in database 
...
async (accessToken, refreshToken, profile, done) => {
  ...
  const userId = `${profile.sub}@${profile.provider}`; //See if document with this unique userId exists in database 
...
})).use(express.static(path.join(__dirname, "client/build"))).use(passport.initialize()).use(passport.session()).use(express.json({
  limit: '20mb'
...
app.get('/users/:userId', async (req, res, next) => {
  ...
  try {
    ...
    if (!thisUser) {
      return res.status(404).send("No user account with id " + req.params.userId + " was found in database.");
    } else {
      return res.status(200).json(JSON.stringify(thisUser));
    }
  } catch (err) {
    console.log();
    return res.status(400).send("Unexpected error occurred when looking up user with id " + req.params.userId + " in database: " + err);
  }
...
app.post('/users/:userId', async (req, res, next) => {
  ...
  if (req.body === undefined || !req.body.hasOwnProperty("type") || !req.body.hasOwnProperty("password") || !req.body.hasOwnProperty("displayName") || !req.body.hasOwnProperty("profilePicURL") || !req.body.hasOwnProperty("securityQuestion") || !req.body.hasOwnProperty("securityAnswer")) {
    ...
    return res.status(400).send("/users POST request formulated incorrectly. " + "It must contain 'type', 'password','displayName','profilePicURL','securityQuestion' and 'securityAnswer fields in message body.");
  }
  ...
  try {
    ...
    if (thisUser) {
      ...
      return res.status(201).send("New account for '" + req.params.userId + "' successfully created.");
    }
  } catch (err) {
...
app.put('/users/:userId', async (req, res, next) => {
  ...
  if (!req.params.hasOwnProperty("userId")) {
    return res.status(400).send("users/ PUT request formulated incorrectly." + "It must contain 'userId' as parameter.");
  }

  const validProps = ['password', 'displayName', 'profilePicURL', 'securityQuestion', 'securityAnswer'];

  for (const bodyProp in req.body) {
    if (!validProps.includes(bodyProp)) {
      return res.status(400).send("users/ PUT request formulated incorrectly." + "Only the following props are allowed in body: " + "'password', 'displayname', 'profilePicURL', 'securityQuestion', 'securityAnswer'");
    }
  }
...
app.delete('/users/:userId', async (req, res, next) => {
  ...
  try {
    ...
    console.log();
    return res.status(400).send("Unexpected error occurred when attempting to delete user account with id " + req.params.userId + ": " + err);
  }
}); /////////////////////////////////
//ROUNDS ROUTES
////////////////////////////////
...
app.post('/rounds/:userId', async (req, res, next) => {
  ...
  if (!req.body.hasOwnProperty("date") || !req.body.hasOwnProperty("course") || !req.body.hasOwnProperty("type") || !req.body.hasOwnProperty("holes") || !req.body.hasOwnProperty("strokes") || !req.body.hasOwnProperty("minutes") || !req.body.hasOwnProperty("seconds") || !req.body.hasOwnProperty("notes")) {
    //Body does not contain correct properties
    return res.status(400).send("POST request on /rounds formulated incorrectly." + "Body must contain all 8 required fields: date, course, type, holes, strokes, " + "minutes, seconds, notes.");
  }
...
app.put('/rounds/:userId/:roundId', async (req, res, next) => {
  ...
  for (const bodyProp in bodyObj) {
    if (!validProps.includes(bodyProp)) {
      return res.status(400).send("rounds/ PUT request formulated incorrectly." + "It includes " + bodyProp + ". However, only the following props are allowed: " + "'date', 'course', 'type', 'holes', 'strokes', " + "'minutes', 'seconds', 'notes'");
    } else {
...
app.delete('/rounds/:userId/:roundId', async (req, res, next) => {
  ...
  try {
    let status = await User.updateOne({
      ...
      $pull: {
        rounds: {
          _id: mongoose.Types.ObjectId(req.params.roundId)
        }
      }
    });
...
app.get('/courses/:courseId', async (req, res, next) => {
  ...
  try {
    let thisUser = await Course.findOne({
      id: req.params.courseId
    });

    if (!thisUser) {
      return res.status(404).send("No user account with id " + req.params.courseId + " was found in database.");
    } else {
      return res.status(200).json(JSON.stringify(thisUser));
    }
  } catch (err) {
    console.log();
    return res.status(400).send("Unexpected error occurred when looking up user with id " + req.params.courseId + " in database: " + err);
  }
...
app.post('/courses/:courseId', async (req, res, next) => {
  ...
  if (!req.body.hasOwnProperty("courseName") || !req.body.hasOwnProperty("rating") || !req.body.hasOwnProperty("review") || !req.body.hasOwnProperty("picture") || !req.body.hasOwnProperty("location") || !req.body.hasOwnProperty("yardage") || !req.body.hasOwnProperty("runningDistance") || !req.body.hasOwnProperty("timePar") || !req.body.hasOwnProperty("bestScore") || !req.body.hasOwnProperty("recordHolder") || !req.body.hasOwnProperty("rateSenior") || !req.body.hasOwnProperty("rateStandard")) {
    //Body does not contain correct properties
    return res.status(400).send("POST request on /course formulated incorrectly." + "Body must contain all 8 required fields: courseName, rating, review, picture, location, yardage, runningDistance, timePar, bestScore, recordHolder, rateSenior, rateStandard.");
  }

  try {
    let thisCourse = await Course.findOne({
      id: req.params.courseId
    });
    ...
    if (thisCourse) {
      ...
      return res.status(200).send("New course for '" + req.params.courseId + "' successfully created.");
    }
  } catch (err) {
...
app.put('/courses/:courseId', async (req, res, next) => {
  ...
  if (!req.params.hasOwnProperty("courseId")) {
    return res.status(400).send("courses/ PUT request formulated incorrectly." + "It must contain 'courseId' as parameter.");
  }
  ...
  for (const bodyProp in req.body) {
    if (!validProps.includes(bodyProp)) {
      return res.status(400).send("courses/ PUT request formulated incorrectly." + "Only the following props are allowed in body: " + "'availability', 'courseName', 'id', 'rating', 'review', 'picture', 'location', 'yardage', 'runningDistance', 'timePar', 'bestScore', 'recordHolder', 'rateSenior', 'rateStandard'");
    }
  }
...
app.delete('/courses/:courseId', async (req, res, next) => {
  ...
  try {
    let status = await Course.deleteOne({
      id: req.params.courseId
    });

    if (status.deletedCount != 1) {
      return res.status(404).send("No course " + req.params.courseId + " was found. Course could not be deleted.");
    } else {
      return res.status(200).send("Course " + req.params.courseId + " was successfully deleted.");
    }
  } catch (err) {
    console.log();
    return res.status(400).send("Unexpected error occurred when attempting to delete course with id " + req.params.courseId + ": " + err);
  }
}); /////////////////////////////////
//APPOINTMENTS ROUTES
////////////////////////////////
...
app.post('/appointments/:userId', async (req, res, next) => {
  ...
  if (!req.body.hasOwnProperty("userId") || !req.body.hasOwnProperty("username") || !req.body.hasOwnProperty("courseName") || !req.body.hasOwnProperty("date") || !req.body.hasOwnProperty("time") || !req.body.hasOwnProperty("paid")) {
    //Body does not contain correct properties
    return res.status(400).send("POST request on /appointments formulated incorrectly." + "Body must contain all 6 required fields: userId, username, courseName, date, time, paid");
  }
  ...
  try {
    ...
    console.log(err);
    return res.status(400).send("Unexpected error occurred when adding appointment" + " to database: " + err);
  }
...
app.put('/appointments/:userId/:appointmentId', async (req, res, next) => {
  ...
  for (const bodyProp in bodyObj) {
    if (!validProps.includes(bodyProp)) {
      return res.status(400).send("appointments/ PUT request formulated incorrectly." + "It includes " + bodyProp + ". However, only the following props are allowed: " + "'userId', 'username', 'courseName', 'date', 'time', 'paid'");
    } else {
...
app.post('/appointments_op/', async (req, res, next) => {
  ...
  if (!req.body.hasOwnProperty("userId") || !req.body.hasOwnProperty("username") || !req.body.hasOwnProperty("courseName") || !req.body.hasOwnProperty("date") || !req.body.hasOwnProperty("time") || !req.body.hasOwnProperty("paid")) {
    //Body does not contain correct properties
    return res.status(400).send("POST request on /appointment_op formulated incorrectly." + "Body must contain all 6 required fields: userId, username, courseName, date, time, paid.");
  }
...
app.put('/appointments_op/:username/:courseName/:date/:time', async (req, res, next) => {
  ...
  try {
    let status = await Appointment.updateOne({
      username: req.params.username,
      courseName: req.params.courseName,
      date: req.params.date,
      time: req.params.time
...
app.post('/cards/:userId', async (req, res, next) => {
  ...
  if (!req.body.hasOwnProperty("name") || !req.body.hasOwnProperty("number") || !req.body.hasOwnProperty("expDate")) {
    //Body does not contain correct properties
    return res.status(400).send("POST request on /cards formulated incorrectly." + "Body must contain all 3 required fields: name, numner, expDate.");
  }
...
app.put('/cards/:userId/:cardId', async (req, res, next) => {
  ...
  for (const bodyProp in bodyObj) {
    if (!validProps.includes(bodyProp)) {
      return res.status(400).send("cards/ PUT request formulated incorrectly." + "It includes " + bodyProp + ". However, only the following props are allowed: " + "'name', 'number', 'expDate'");
    } else {
